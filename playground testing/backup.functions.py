...
def file_exists_in_openai(file_name):
    files = client.files.list(purpose="assistants")  # You may need to adjust the purpose
    return any(file["filename"] == file_name for file in files.data)

# Modify the watch_folder_and_upload function
def watch_folder_and_upload():
    while True:
        # Create a Thread with an initial user message
        thread = client.beta.threads.create(messages=[
            {
                "role": "user",
                "content": "Use this file to respond to the user query"
            }
        ])
        ...
        for file_name in files:
            ...
            if not file_exists_in_openai(file_name):
                file_path = os.path.join(folder_path, file_name)

                # Upload the file to OpenAI using the created thread_id
                upload_file_to_openai(file_path, thread.id)

        # Add a delay before checking for new files again
        time.sleep(60)

# Uncomment the line below to start watching for new files and uploading them
watch_folder_and_upload()

def upload_file_to_openai(file_path, thread_id):
    try:
        with open(file_path, "rb") as file:
            ...
            print(f"Uploaded file: {response['filename']} to thread: {thread_id}")
    except OpenAIError as e:
